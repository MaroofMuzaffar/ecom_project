...
import { updateTestmonials, getTestmonials } from "../../Redux/ActionCreators/TestmonialsActionCreators"
export default function AdminUpdateTestmonials() {
  let { id } = useParams()
  ...
  function postData(e) {
    ...
    if (error)
      ...
      let item = TestmonialsStateData.find((x) => x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase() && x.id !== id)
      if (item) {
        setShow(true)
        setErrorMessage((old) => {
          return {
        ...
        dispatch(updateTestmonials({ ...data }))
  ...
  useEffect(() => {
    (() => {
      ...
      if (TestmonialsStateData.length)
        setData(TestmonialsStateData.find((x) => x.id === id))
    })()
      ...
      <div className="container-fluid">
        <div className='row'>
          ...
          <div className="col-md-9">
            <h5 className='bg-primary text-center text-light p-2'>Update Testmonials <Link to="/admin/testmonials"> <i className='fa fa-backward text-light float-end'></i></Link></h5>
            <form onSubmit={postData}>

              <div className="mb-3">
                <label>Name*</label>
                <input type="text" name="name" value={data.name} onChange={getInputData} className={`form-control border-3 ${show && errorMessage.name ? 'border-danger' : 'border-primary'}`} placeholder='Testmonials Name' />
