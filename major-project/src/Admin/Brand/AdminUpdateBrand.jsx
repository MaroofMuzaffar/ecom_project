...
export default function AdminUpdateBrand() {
  ...
  function postData(e) {
    ...
    if (error)
      ...
      let item = BrandStateData.find((x) => x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase() && x.id !== id)
      if (item) {
        setShow(true)
        setErrorMessage((old) => {
          return {
        ...
        <div className='row'>
          ...
          <div className="col-md-9">
            ...
            <form onSubmit={postData}>
              ...
              <div className="mb-3">
                <label>Name*</label>
                <input type="text" name="name" value={data.name} onChange={getInputData} className={`form-control border-3 ${show && errorMessage.name ? 'border-danger' : 'border-primary'}`} placeholder='Brand Name' />
