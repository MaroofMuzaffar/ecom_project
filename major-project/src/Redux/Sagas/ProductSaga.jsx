...
function* createSaga(action) {
    let response = yield createRecords("product", action.payload)
    yield put({ type: CREATE_PRODUCT_RED, payload: response })
}
function* getSaga() {
    let response = yield getRecords("product")
    yield put({ type: GET_PRODUCT_RED, payload: response })
}
function* updateSaga(action) {
    yield updateRecords("product", action.payload)
    yield put({ type: UPDATE_PRODUCT_RED, payload: action.payload })
}

function* deleteSaga(action) {
    yield deleteRecords("product", action.payload)
